...
fn main() -> Result<(), Box<dyn std::error::Error>>{
    ...
    let mut func: 
        fn() -> Result<(Scene, Camera), Box<dyn std::error::Error>> = final_scene;
    if args.len() > 1 {
        ...
        match shading_mode {
            ...
            Ok(8) => func = cornell_smoke,
            Ok(9) => func = final_scene,
            _ => {
...
fn cornell_smoke() -> Result<(Scene, Camera), Box<dyn std::error::Error>>{
    let mut world = HittableList::new();
    let red   = Material::Lambertian { albedo: Texture::from((0.65, 0.05, 0.05)) };
    let white = Material::Lambertian { albedo: Texture::from((0.73, 0.73, 0.73)) };
    let green = Material::Lambertian { albedo: Texture::from((0.12, 0.45, 0.15)) };
    let light = Material::DiffuseLight(
        Texture::SolidColor(vec3::new(7., 7., 7.))
    );
    ...
    let box1 = ConstantMedium::new(
        box1,
        0.01,
        Texture::SolidColor(vec3::new(0., 0., 0.))
    );
    world.add(box1);
    ...
    let box2 = ConstantMedium::new(
        box2,
        0.01,
        Texture::SolidColor(vec3::new(1., 1., 1.))
    );
    world.add(box2);
...
fn final_scene() -> Result<(Scene, Camera), Box<dyn std::error::Error>>{
    let mut world = HittableList::new();
    let ground = Material::Lambertian { albedo: vec3::new(0.48, 0.83, 0.53).into() };
    ...
    for (j, i) in (0..BOXES_PER_SIDE).cartesian_product(0..BOXES_PER_SIDE)
    {
        ...
        let y0 = 0.;
        let x1 = x0 + w;
        let y1 = rng.gen_range(1f64..100.);
        let z1 = z0 + w;

        world.add(QuadBox::new(
            vec3::new(x0, y0, z0),
            vec3::new(x1, y1, z1),
            ground.clone()
        ));
    ...
    let sphere_material = Material::Lambertian { albedo: vec3::new(0.7, 0.3, 0.1).into() };
    world.add(Sphere{
        center: vec3::new(400., 400., 200.),
        radius: 50.,
        material: sphere_material,
        move_to: Some(vec3::new(30., 0., 0.)),
    });
    ...
    world.add(Sphere{
        center: vec3::new(0., 150., 45.),
        radius: 50.,
        material: Material::Metal { albedo: vec3::new(0.8, 0.8, 0.9), fuzz: 1.0 },
        move_to: None,
    });
    ...
    world.add(
        ConstantMedium::new(
            Shapes::Sphere(boundary),
            0.2,
            vec3::new(0.2, 0.4, 0.9).into(),
        )
    );
    let boundary = Sphere{
        center: vec3::ZERO,
    ...
    let emat = Material::Lambertian{ albedo: Texture::load_image("assets/earthmap.jpg")? };
    world.add(Sphere{
    ...
    let pertext = Texture::PerlinNoise(Perlin::default(), 0.2);
    world.add(Sphere{
    ...
    let mut sphere_list = HittableList::new();
    let white = Material::Lambertian { albedo: vec3::splat(0.73).into() };
    let ns = 1000;
    for _ in 0..ns {
    ...
    let sphere_list = Shapes::set_rotate_y(Shapes::Collection(sphere_list), 15.);
    let sphere_list = Shapes::set_translate(
        sphere_list,
        vec3::new(-100., 270., 395.),
    );
    world.add(sphere_list);
...
impl Hittable for Shapes {
    fn hit(
        ...
        match self {
            ...
            Shapes::Collection(list) => {
                list.hit(ray, interval)
            },
        }
    }
}
...
impl Hittable for ConstantMedium {
    fn hit(
        ...
        rec1.t = rec1.t.max(interval.start);
        rec2.t = rec2.t.min(interval.end);
        ...
        let hit_distance =
            self.neg_inv_density * rng.gen::<f64>().log2(); // can also try log10()
