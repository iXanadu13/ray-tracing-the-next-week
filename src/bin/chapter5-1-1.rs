...
impl Default for Perlin{
    fn default() -> Self {
        let mut rng = rand::thread_rng();
        let mut res: [[[f64; 4]; 4]; 4] = [[[0.; 4]; 4]; 4];

        res.iter_mut().for_each(|inner_array| {
            inner_array.iter_mut().for_each(|sub_array| {
                sub_array.iter_mut().zip((0..).map(|_| rng.gen())).for_each(|(e, v)| {
                    *e = v;
                });
            });
        });
...
impl Perlin{
    fn get(&self, point: vec3) -> f64 {
        let i = (4. * point.x).abs() as usize;
        let j = (4. * point.y).abs() as usize;
        let k = (4. * point.z).abs() as usize;
        self.rand[i % 4][j % 4][k % 4]
    }
}
