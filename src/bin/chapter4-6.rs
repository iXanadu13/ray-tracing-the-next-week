...
use std::{ops::Range,ops::Neg, fs, f64::consts::PI, path::Path, env};
...
fn main() -> Result<(), Box<dyn std::error::Error>>{
    ...
    if args.len() > 1{
        ...
        match shading_mode {
            Ok(1) => {
                let builder = bouncing_spheres(&mut world)?;
                let camera = builder.build();
                camera.render(&world)?;
            }
            Ok(2) => {
                let builder = checker_spheres(&mut world)?;
                let camera = builder.build();
                camera.render(&world)?;
            },
            Ok(3) => {
                let builder = earth(&mut world)?;
                let camera = builder.build();
                camera.render(&world)?;
            }
            _ => {
